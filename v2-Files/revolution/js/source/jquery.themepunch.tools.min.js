...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	_gsScope._gsDefine("easing.Back", ["easing.Ease"], function(Ease) {
		...
		var w = (_gsScope.GreenSockGlobals || _gsScope),
			...
			_createBack = function(n, f) {
				var C = _class("easing." + n, function(overshoot) {
						this._v2 = (overshoot || overshoot === 0) ? overshoot : 1.70158;
						this._p2 = this._v2 * 1.525;
					}, true), 
					p = C.prototype = new Ease();
				p.constructor = C;
				p.getRatio = f;
			...
			Back = _wrap("Back",
				_createBack("BackOut", function(p) {
					return ((p = p - 1) * p * ((this._v2 + 1) * p + this._v2) + 1);
				}),
				_createBack("BackIn", function(p) {
					return p * p * ((this._v2 + 1) * p - this._v2);
				}),
			...
			SlowMo = _class("easing.SlowMo", function(linearRatio, power, yoyoMode) {
				...
				this._v2 = (1 - linearRatio) / 2;
				this._p2 = linearRatio;
				this._p3 = this._v2 + this._p2;
		...
		p.getRatio = function(p) {
			...
			if (p < this._v2) {
				return this._calcEnd ? 1 - ((p = 1 - (p / this._v2)) * p) : r - ((p = 1 - (p / this._v2)) * p * p * p * r);
			} else if (p > this._p3) {
				return this._calcEnd ? 1 - (p = (p - this._p3) / this._v2) * p : r + ((p - r) * (p = (p - this._p3) / this._v2) * p * p * p);
			}
		...
		_createElastic = function(n, f, def) {
			var C = _class("easing." + n, function(amplitude, period) {
					this._v2 = (amplitude >= 1) ? amplitude : 1; //note: if amplitude is < 1, we simply adjust the period for a more natural feel. Otherwise the math doesn't work right and the curve starts at 1.
					this._p2 = (period || def) / (amplitude < 1 ? amplitude : 1);
					this._p3 = this._p2 / _2PI * (Math.asin(1 / this._v2) || 0);
		...
		_wrap("Elastic",
			_createElastic("ElasticOut", function(p) {
				return this._v2 * Math.pow(2, -10 * p) * Math.sin( (p - this._p3) * this._p2 ) + 1;
			}, 0.3),
			_createElastic("ElasticIn", function(p) {
				return -(this._v2 * Math.pow(2, 10 * (p -= 1)) * Math.sin( (p - this._p3) * this._p2 ));
			}, 0.3),
			_createElastic("ElasticInOut", function(p) {
				return ((p *= 2) < 1) ? -0.5 * (this._v2 * Math.pow(2, 10 * (p -= 1)) * Math.sin( (p - this._p3) * this._p2)) : this._v2 * Math.pow(2, -10 *(p -= 1)) * Math.sin( (p - this._p3) * this._p2 ) * 0.5 + 1;
			}, 0.45)
		);
